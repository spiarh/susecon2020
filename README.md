...
Default password: `nextcloud` is used everywhere:
...
zypper --gpg-auto-import-keys in -y libvirt terraform terraform-provider-libvirt go1.14 make
...
```bash
kubectl delete -f caasp-cluster/addons/cni/cilium.yaml
...
$ kubectl get nodes 
NAME                 STATUS   ROLES    AGE   VERSION
master-0   Ready    master   11d   v1.16.2
worker-0   Ready    <none>   11d   v1.16.2
```

And we want to expose Envoy as the frontend proxy on `worker-0`,
we can retrieve its IP:

```bash
$ kubectl get node worker-0 -o jsonpath='{.status.addresses[?(@.type=="InternalIP")].address}'
192.168.0.132
```
...
```bash
export EXPOSE_IP=$(kubectl get node worker-0 -o jsonpath='{.status.addresses[?(@.type=="InternalIP")].address}')
echo "$EXPOSE_IP envoy prestashop mariadb nextcloud grafana prometheus hubble prestashop.susecon.lab mariadb.susecon.lab nextcloud.susecon.lab grafana.susecon.lab prometheus.susecon.lab hubble.susecon.lab envoy.susecon.lab" >> /etc/hosts
```
...
kube-system   etcd-lcavajani-master-0                          1/1     Running   0          11d
...
kube-system   kube-apiserver-lcavajani-master-0                1/1     Running   0          11d
kube-system   kube-controller-manager-lcavajani-master-0       1/1     Running   0          11d
...
kube-system   kube-scheduler-lcavajani-master-0                1/1     Running   0          11d
